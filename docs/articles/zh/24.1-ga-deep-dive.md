...
> 这篇文章是 ares 进化引擎的完整介绍。模块路径 `internal/ares_evolution/genome/`。我们用的符号都是仓库里真实存在的：`Population`、`doEvolve`、`TournamentSelection`，`Selection` 接口下有 7 个算子，交叉有 3 种 `CrossoverType` + 3 种 `PromptCrossoverMode`，变异有 5 种 `MutationType`，多目标走 NSGA-II（`ParetoDominance` / `CrowdingDistance`）。所有数字都是源码里的常量或配置默认值。
...
```text
internal/ares_evolution/        # GA 种群引擎本身（本篇文章舞台）
internal/evolution/             # 运行时进化引擎（Genome/Diff/Coordinator）
internal/ares_flight/           # Agent 家谱树 / 谱系（见 24.5）
```

需要特别澄清一点（这也是旧版文章的错误）：**默认配置里没有硬编码"锦标赛"选择策略**。`internal/ares_evolution/genome/population_config.go` 的 `DefaultPopulationConfig()` 把 `SelectionStrategy` 留空（`""`），在 `buildSelector()` 里空字符串等价于 `"random"`——父代是从繁殖池里纯随机挑的。公开 API `api/evolution` 的 `DefaultPopulationConfig()` 才是 `"tournament"`。两套默认值不同，读者不要混淆。
...
flowchart LR
    ...
    subgraph core["internal/ares_evolution/genome（核心引擎）"]
    ...
    subgraph rt["internal/evolution（运行时进化，Ch.8 候选管线）"]
...
设计上的核心约束在 `Genome` 接口文档里写得很清楚（`internal/evolution/genome/genome.go`）：
...
`internal/ares_evolution/genome/population.go` 定义了核心结构。字段比旧版文章里我写过的更丰富：
...
全部在 `internal/ares_evolution/genome/selection.go`。每个都实现统一的 `Selection` 接口：
...
### 交叉（`internal/ares_evolution/genome/crossover.go`）
...
### 变异（`internal/ares_evolution/mutation/types.go`）
...
`internal/ares_evolution/genome_wiring_system.go` 提供 `NewWiredEvolutionSystem(...)` 中央工厂，把 Population、scorer、谱系、guardrails、diff、coordinator 串起来；`RunIdleEvolution(ctx, system, n)` 跑 N 代，每代大致是：

```text
评分 → 进化 → 谱系记录 → LLM 反射 → 假说生成 → 差分引擎 → 协调器 → 代后钩子
```

这部分把 GA 接到 `internal/evolution` 的运行时进化管线（见下一节）。

---

## 八、运行时进化：Genome / Diff / Coordinator（Ch.8 候选管线）

`internal/evolution/genome/workflow_genome.go` 是**真正的运行时 DAG 基因组**：
...
候选指令侧，`internal/evolution/ga_generator.go` 的 `GAGenerator` 把角色的 stable instructions 当父代做 prompt 变异，产出 `Candidate`。`CandidatePipeline.Release`（`candidate_pipeline.go`）把**已验证**的候选送进协调器决策 + 部署的金丝雀管线：
...
协调器的默认策略 `coordinator.DefaultPolicy()`（`internal/evolution/coordinator/coordinator.go`）是真实常量：
...
| 内容 | 路径 |
|------|------|
| 人口与进化主循环 | `internal/ares_evolution/genome/population.go` |
| 默认配置 | `internal/ares_evolution/genome/population_config.go` |
| 选择算子 | `internal/ares_evolution/genome/selection.go` |
| 交叉/继承 | `internal/ares_evolution/genome/crossover.go` |
| 多目标 | `internal/ares_evolution/genome/multi_objective.go` |
| 变异类型 | `internal/ares_evolution/mutation/types.go` |
| 变异引擎 | `internal/ares_evolution/mutation/mutator.go` |
| 公开 API | `api/evolution/evolution.go`, `api/evolution/mutation/mutator.go` |
| 编排工厂 | `internal/ares_evolution/genome_wiring_system.go` |
| 运行时 DAG 基因组 | `internal/evolution/genome/workflow_genome.go` |
| 候选生成与发布 | `internal/evolution/ga_generator.go`, `candidate_pipeline.go` |
| 协调器 | `internal/evolution/coordinator/coordinator.go` |

[B] 想亲手跑：`go run ./examples/10-ga-full-evolution`。
