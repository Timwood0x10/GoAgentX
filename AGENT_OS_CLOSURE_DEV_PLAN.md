...
- **状态更新（2026-09-05，仅追加）**：N-2 的"单 agent 内 DAG"闭合路径已从 Y.1 方案 C 迁移到主线文档 `TOOL_DAG_MAINLINE_DESIGN.md`（节点 = 一次工具执行，L1/L2 同构 `engine.MutableDAG`）。主线 **M0（增量编译器）+ M1（L2 图容器 + tool/answer 执行体 + router 派发，走真 Scheduler）已落地**（2026-09-05，见主线 §9）；M2（planner 生长节点）起才真正取代 ReAct 消息循环。**N-2 在 M4 删除 `chatStepState` 前维持未闭环**，发布措辞仍维持"进化作用于 peer 级 agent 拓扑"，不得因 M0/M1 落地而提前改写。
...
**状态注记（2026-09-05，仅追加，不改原文）**：M.3 方向草案已升格为独立主线文档 `TOOL_DAG_MAINLINE_DESIGN.md`（两层同构 MutableDAG、M0–M6 落地序列，前置约束已折入各里程碑验收行），本 M.3 保留为草案历史。后续实施以主线文档为唯一依据。
